...
namespace Moritz.Score
{
    public class SvgWriter : XmlWriterWrapper
    {
        ...
        public void SvgDrawFlagBlock(Metrics flagsBlockMetrics, float fontSize)
        {
            string transformString =
                "translate(" + 
                flagsBlockMetrics.OriginX.ToString(M.En_USNumberFormat) + "," + 
                flagsBlockMetrics.OriginY.ToString(M.En_USNumberFormat) + ") " +
                "scale(" + fontSize.ToString(M.En_USNumberFormat) + ")";

            _w.WriteStartElement("g");
            _w.WriteStartElement("use");
            _w.WriteAttributeString("xlink", "href", null, "#" + flagsBlockMetrics.ID_Type);
            _w.WriteAttributeString("transform", transformString);
            _w.WriteEndElement(); // use
            _w.WriteEndElement(); // g
        }
