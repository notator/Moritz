...
namespace Moritz.Spec
{
    ...
    /// <para>foreach(MidiChordDef midiChordDef in trkDef.MidiChordDefs) { ... }</para>
    ...
    public class Trk : ICloneable
    {
        ...
        public Tuple<Trk, Trk, double?> PopTrk(int poppedMsDuration)
        {
            ...
            double? finalSplitProportion = null;
        ...
        //        Debug.Assert(value > MsDuration);

        //        IUniqueDef lastLmdd = _uniqueDefs[_uniqueDefs.Count - 1];
        //        lastLmdd.MsDuration = value - MsDuration;
        ...
        {
            int returnedIndex = -1;
            if(msPositionReFirstIUD == MsDuration)
            {
                returnedIndex = this.Count;
            }
            else if(msPositionReFirstIUD >= _uniqueDefs[0].MsPositionReFirstUD && msPositionReFirstIUD < MsDuration)
            {
        ...
        public virtual void SetVelocitiesFromDurations(int velocityForMinMsDuration, int velocityForMaxMsDuration, double percent = 100.0)
        {
            ...
            foreach(IUniqueDef iud in _uniqueDefs)
            {
                ...
                {
                    msDurationRangeMin = (msDurationRangeMin < midiChordDef.MsDuration) ? msDurationRangeMin : midiChordDef.MsDuration;
                    msDurationRangeMax = (msDurationRangeMax > midiChordDef.MsDuration) ? msDurationRangeMax : midiChordDef.MsDuration;
                }
            }
            ...
            foreach(IUniqueDef iud in _uniqueDefs)
            {
                if(iud is MidiChordDef midiChordDef)
                {
                    midiChordDef.SetVelocityFromDuration(msDurationRangeMin, msDurationRangeMax, velocityForMinMsDuration, velocityForMaxMsDuration, percent);
                }
            }
        }
        ...
        public void SetVerticalVelocityGradient(int rootVelocity, int topVelocity)
        {
            ...
            foreach(IUniqueDef iud in UniqueDefs)
            {
                if(iud is MidiChordDef midiChordDef)
                {
                    midiChordDef.SetVerticalVelocityGradient(rootVelocity, topVelocity);
                }
            }
        }
        ...
        public void SetDurationsFromPitches(int durationForLowestPitch, int durationForHighestPitch, bool useBottomPitch, double percent = 100.0)
        {
            ...
            foreach(IUniqueDef iud in _uniqueDefs)
            {
                if(iud is MidiChordDef midiChordDef)
                {
                    int pitch = (useBottomPitch == true) ? midiChordDef.Pitches[0] : midiChordDef.Pitches[midiChordDef.Pitches.Count - 1];
            ...
            if(pitches.Count == 1)
            {
                ...
                foreach(IUniqueDef iud in _uniqueDefs)
                {
                    if(iud is MidiChordDef midiChordDef)
                    {
                        midiChordDef.MsDuration = mcdMsDuration;
                    }
                }
            }
            else
            {
                ...
                foreach(IUniqueDef iud in _uniqueDefs)
                {
                    ...
                    {
                        int pitch = (useBottomPitch == true) ? midiChordDef.Pitches[0] : midiChordDef.Pitches[midiChordDef.Pitches.Count - 1];
                        int oldDuration = midiChordDef.MsDuration;
                        int durPerPitch = msDurPerPitch[pitch];
                        midiChordDef.MsDuration = (int)Math.Round((oldDuration * factorForOldValue) + (durPerPitch * factorForNewValue));
                    }
                }
        ...
        public void AdjustExpression(int beginIndex, int endIndex, double factor)
        {
            if(CheckIndices(beginIndex, endIndex))
            {
                for(int i = beginIndex; i < endIndex; ++i)
                {
                    ...
                    {
                        iumdd.MidiChordControlDef.Expression = (int)Math.Round((int)iumdd.MidiChordControlDef.Expression * factor);
                    }
                }
            }
        }
        ...
        public virtual void AdjustVelocitiesHairpin(int beginIndex, int endIndex, double startFactor, double endFactor)
        {
            if(CheckIndices(beginIndex, endIndex))
            {
                ...
                if(steps > 0)
                {
                    ...
                    for(int i = beginIndex; i < endIndex; ++i)
                    {
                        MidiChordDef midiChordDef = _uniqueDefs[i] as MidiChordDef;
        ...
        public void SetPitchWheelDeviation(int beginIndex, int count, int deviation)
        {
            ...
            if(CheckIndices(beginIndex, endIndex))
            {
                for(int i = beginIndex; i < endIndex; ++i)
                {
                    if(this[i] is MidiChordDef midiChordDef)
                    {
                        midiChordDef.MidiChordControlDef.VelocityPitchSensitivity = M.MidiValue(deviation);
                    }
                }
            }
        }
        ...
        public void RemoveScorePitchWheelCommands(int beginIndex, int endIndex)
        {
            if(CheckIndices(beginIndex, endIndex))
            {
                for(int i = beginIndex; i < endIndex; ++i)
                {
                    ...
                    {
                        umcd.MidiChordControlDef.PitchWheel = null;
                    }
                }
            }
        }
        ...
        protected void AdjustPitchWheelDeviations(int beginIndex, int endIndex, int pwValueAtBeginIndex, int pwValueAtEndIndex)
        {
            ...
            for(int i = beginIndex; i < endIndex; ++i)
            {
                ...
                {
                    umc.MidiChordControlDef.PitchWheelSensitivity = M.MidiValue(pwValueAtBeginIndex * (Math.Pow(pwdfactor, i)));
                }
            }
        }
        ...
        /// The Trk.MsDuration is calculated using the MsDuration fields of its UniqueDefs, except that if MsDurationToNextBarline
        /// field on the final UniqueDef is != null, then that value is used instead.
        ...
        public int MsDuration
        {
            get
            {
                ...
                foreach(IUniqueDef iud in _uniqueDefs)
                {
                    ...
                    {
                        total += (int)mcd.MsDurationToNextBarline;
                    }
                    else
                    {
                        total += iud.MsDuration;  // also CautionaryChordDef
                    }
            ...
            set
            {
                ...
                foreach(IUniqueDef iud in _uniqueDefs)
                {
                    if(iud is MidiChordDef mcd && mcd.MsDurationToNextBarline != null)
                    {
                        relativeDurations.Add((int)mcd.MsDurationToNextBarline);
                    }
                    else
                    {
                        relativeDurations.Add(iud.MsDuration);   // also CautionaryChordDef
                    }
                }

                List<int> newDurations = M.IntDivisionSizes(newMsDuration, relativeDurations);
                ...
                foreach(IUniqueDef iud in _uniqueDefs)
                {
                    if(iud is MidiChordDef mcd && mcd.MsDurationToNextBarline != null)
                    {
                        mcd.MsDurationToNextBarline = newDurations[i];
                    }
                    else
                    {
                        relativeDurations.Add(newDurations[i]);  // also CautionaryChordDef
                    }
                    newTotal += newDurations[i];
                    ++i;
                }

                Debug.Assert(newMsDuration == newTotal);
        ...
        internal void ResetMsPositionsReFirstUID()
        {
            int msPos = 0;
            foreach(var uid in _uniqueDefs)
            {
                uid.MsPositionReFirstUD = msPos;
                msPos += uid.MsDuration;                
            }
        }
